...
std::vector<double>
compute_distance_from_centroid(GeometryData &geometry_data,
...
int main(int argc, char *argv[]) {
  ...
  command_line_options.add_options()(
      ...
      "output text file - containing x, y, z, and distance")("help",
                                                             "Print usage");
  ...
  try {
    ...
    std::vector<double> distances =
        compute_distance_from_centroid(geometry_data, centroid);
  ...
  std::cout << "Practice_03: duration: " << end_time - start_time << " seconds"
            << std::endl;
  return 0;
}
