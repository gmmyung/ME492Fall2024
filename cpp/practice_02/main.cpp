...
int main(int argc, char *argv[]) {
  ...
  command_line_options.add_options()("help", "Print usage");
  ...
  try {
    ...
    std::cout << "Dot product: " << mfs.compute_dot_product(vector_u, vector_v)
              << std::endl;
    std::array<double, 3> cross_vector =
        mfs.compute_cross_product(vector_u, vector_v);
    std::cout << "Cross product: " << cross_vector[0] << " " << cross_vector[1]
              << " " << cross_vector[2] << std::endl;
  }
  ...
  std::cout << "Practice_01: duration: " << end_time - start_time << " seconds"
            << std::endl;
  return 0;
}
